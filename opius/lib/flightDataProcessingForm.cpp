...
//		if (!(n_fld=GetParmVec(str, p_fld, ':')))             // Net nolei v stpoke faila ciklogpammu?
		...
		{	show_error(UnicodeString("\n N bopta ciklogpamma -> " + IntToStr(static_cast<int> (bort_tmp)) +" != ") );
			show_error(UnicodeString(IntToStr(static_cast<int>(OD.Bort)) + " <- OD\n"));
...
//	info_str->Add(" Visualization of parameteres code values bYP ");
	...
	{ show_error(UnicodeString(" Channels aren't entered. (can't visualize parameter codes)"));}
}
	...
	TStrings* checkStrings,	TStrings* ChannelBox_strings)
{
	...
	// вывод параметров // строго меньше
	for( int par_num = 0; par_num < maxChannel; par_num++ ) {
		anlg_par_memo->Add("");

		// шапка (номер параметра)
		anlg_par_grid->Cells[par_num+1][0] = IntToStr(par_num);
		anlg_par_grid->ColCount += 1;
		...
		if( p_AP[par_num]==0 )
			...
			del_dup_spaces(anlg_param.name);

			String itemGraph = IntToStr(anlg_param.number) + ". " +
				String(anlg_param.id.c_str()) + " ("
				+ String(anlg_param.name.c_str()) + ")";
			checkStrings->Add(itemGraph);
			ChannelBox_strings->Add(itemGraph);

			anlg_par_memo->Add(" Parameter #" + IntToStr(par_num));
			anlg_par_memo->Add("\n Identifier == "
									+ String(p_AP[par_num]->p_id));
			anlg_par_memo->Add("\n Name == " + String(p_AP[par_num]->p_nm));
			anlg_par_memo->Add("\n \"type\" == " + IntToStr(p_AP[par_num]->tp));
			anlg_par_memo->Add("\n 'bYp' channel == "
									+ IntToStr(p_AP[par_num]->cn));
			anlg_par_memo->Add("\n Registration rate in frame == "
									+ IntToStr(p_AP[par_num]->fo));
			anlg_par_memo->Add("\n Base channel registration of analog"
								" parameter in frame == "
									+ IntToStr(p_AP[par_num]->cnm));
			anlg_par_memo->Add("\n processing flag == "
									+ IntToStr(p_AP[par_num]->fl_obr));

			anlg_param.max_val = p_AP[par_num]->fz[0];
			anlg_param.min_val = p_AP[par_num]->fz[0];

			for (n_kdr=0; n_kdr < dl_fk; n_kdr++)
			{ // значения параметра во времени (по кадрам)
				anlg_par_grid->Cells[par_num+1][n_kdr+1]
							= FloatToStr(p_AP[par_num]->fz[n_kdr]);
...
//	check_sc_strings->Clear();
	...
	*/
	...
	for( int channel = 0; channel < maxChannel; channel++ ) { // строго меньше
		...
		if( p_RK[channel]== 0 ) {
			sngl_com_memo->Add(" Single commands in channel #"
				+ IntToStr(channel) + " is absent");
		} else {
			sngl_com_memo->Add(" Single commands in channel #"
				+ IntToStr(channel));

			sngl_com_memo->Add(" type == " + IntToStr(p_RK[channel]->tp)
				+ " (" + single_command_type(p_RK[channel]->tp).c_str() + ")");
			sngl_com_memo->Add(" 'bYp' channel == "
								+ IntToStr(p_RK[channel]->cn));
			sngl_com_memo->Add(" Registration rate in frame == "
								+ IntToStr(p_RK[channel]->fo));
			...
			for( int com_num = 0; com_num < commands_num; com_num++ ) {
				...
				if( p_RK[channel]->fz[com_num] == 0 )
					sngl_com_memo->Add(" Single command #" + IntToStr(com_num)
						+ " in channel #" + IntToStr(channel) + " is absent");
				else {
					Single_command sc;
					sc.channel = channel;
					sc.number = com_num;
					sc.id = p_RK[channel]->p_id[com_num];
					sc.name = p_RK[channel]->p_nm[com_num];
					del_dup_spaces(sc.name);
					...
					{
						sngl_com_grid->Cells[cur_cell_col][0] =
						(header_output_count^=1) ? // циклическое изменение счетчика
						IntToStr(channel) : UnicodeString("channel");
					}
					sngl_com_grid->Cells[cur_cell_col][1] = "CMD"
													+ IntToStr(com_num);
					sngl_com_grid->ColCount += 1;

					sc.max = p_RK[channel]->fz[com_num][0];
					sc.min = p_RK[channel]->fz[com_num][0];
					for (n_kdr=0; n_kdr < dl_fk; n_kdr++) {
						char cur_val = p_RK[channel]->fz[com_num][n_kdr];
						sngl_com_grid->Cells[cur_cell_col]
							[n_kdr + title_rows_count] = static_cast<int>(cur_val);
						sc.values.push_back(cur_val != 0);
						if(cur_val > sc.max) {
                            sc.max = cur_val;
						}
						if(cur_val < sc.min) {
					...
					if( ++cur_cell_col < 0 ) {
						show_error(UnicodeString(" Error @ PrintFreeZone_v2()"
								" : current cell column number overflow"));
					}

					sngl_com_memo->Add(" Single command #" + IntToStr(com_num)
						+ " in channel #" + IntToStr(channel));
					sngl_com_memo->Add(UnicodeString(" name == ")
						+ p_RK[channel]->p_nm[com_num]);
					sngl_com_memo->Add(UnicodeString(" id == ")
						+ p_RK[channel]->p_id[com_num]);

					/*
					// min & max are reversed ...
					sngl_com_memo->Add(" max val == "
						+ FloatToStr (p_RK[channel]->par_min[com_num]));
					sngl_com_memo->Add(" min val == "
						+ FloatToStr (p_RK[channel]->par_max[com_num]));
					*/

					sngl_com_memo->Add(" max val == " + IntToStr(int(sc.max)));
					sngl_com_memo->Add(" min val == " + IntToStr(int(sc.min)));
					sngl_com_memo->Add(" 'IRK' discharge == "
						+ IntToStr (p_RK[channel]->rz[com_num]));
					sngl_com_memo->Add(" processing flag == "
						+ IntToStr (p_RK[channel]->fl_obr[com_num]));
					sngl_com_memo->Add(" Begining frame number == "
						+ IntToStr (p_RK[channel]->nkdr_n[com_num]));
					sngl_com_memo->Add(" End frame number == "
						+ IntToStr (p_RK[channel]->nkdr_k[com_num]));
					sngl_com_memo->Add(" 'IRK' delay of forming == "
						+ IntToStr (p_RK[channel]->zdf[com_num]));

					if(sc.min == sc.max && sc.max == false) {
						sc.state = Single_command::State::null;
					} else if(sc.min == sc.max && sc.min == true) {
						sc.state = Single_command::State::constant;
					} else {
						sc.state = Single_command::State::variable;
					}

					sc.color = optimal_rand_color();
					sngl_cmd_vec.push_back(sc);
				}
			}
		}
	...
	if( sngl_com_grid->ColCount > title_cols_count ) {
		...
		sngl_com_grid->ColCount = 0;
		sngl_com_grid->RowCount = 0;
		/*
		sngl_com_memo->Insert(0, "Error @ Output_sngl_cmd() :"
			" Can't create header column - not enough columns "
			" (! sngl_com_grid->ColCount > title_cols_count)");
		*/
		return;
		/*
		show_error(UnicodeString(" Error @ Output_sngl_cmd() :"
			" Can't create header column - not enough columns "
			"\n (! sngl_com_grid->ColCount > title_cols_count) "));
		*/
    }
...
int data_load(const String fileCopy, const String cyclogram)
{
	...
	{
		show_error(UnicodeString(" Some unhandled exception... "));
		Application->ShowException(&exc);
		return FL_ERR;
	}
	catch(...)
	{
		show_error(UnicodeString(" Some unknown exception... "));
		return FL_ERR;
    }
...
std::string single_command_type(short tp) {
	switch(tp) {
	case 0 : return "slugebNiy napametp";
	case 1 : return "aNalogoviy napametp";
	case 2 : return "aNalogoviy napametp paschetNy";
	case 3 : return "pazovaja komaNda osNovNaja (dvoichNaja)";
	case 4 : return "pazovaja komaNda aNalogovaja";
	case 5 : return "pazovaja komaNda iskustveNNaja (paschetNaja)";
