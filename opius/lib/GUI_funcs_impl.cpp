...
void show_error(const UnicodeString& msg)
{
	//ShowMessageUser(UnicodeString(msg.c_str()));
...
bool checkFileName(String fileName, int fileType) {
	if( !FileExists(fileName) ) {
		UnicodeString str = fileType ? "Cyclogram" : "File-Copy";
...
int Color_generator::generate_rand_color(const int max_bright, int max_iteration)
{
	...
	do
	{
		color = TColor(RGB(rand()% max_color_intensity, rand()% max_color_intensity,
...
void del_dup_spaces(std::string& str) {
