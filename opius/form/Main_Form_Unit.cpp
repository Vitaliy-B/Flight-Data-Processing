...
std::vector<Analog_parameter> anlg_param_vec;
...
{
	//zoom_reset_btnClick(0); // to correct scale, but now unnecessary
}
...
void __fastcall TMainGUI::btn_sc_add_rnd_Click(TObject *Sender)
{
	if(data_loaded())
	{
		...
		else
		{
		  ...
		  output_label->Caption = sc_min_max_to_str(unchecked[random]);
		}
	}
}
	...
	if(data_loaded())
	{
		 try {
			...
			Chart_sc->BottomAxis->SetMinMax(
				std::min(Chart_sc->MinXValue(Chart_sc->BottomAxis), 0.0),
				std::max(Chart_sc->MaxXValue(Chart_sc->BottomAxis), double(dl_fk)));
...
// correctly deletes chart series with custom axis
void TMainGUI::delete_chart_series(TChartSeries* series) {
	TChartAxis* axis = series->CustomVertAxis;
	if(axis != 0) {
		series->CustomVertAxis = 0;
		dynamic_cast<TChart*>(series->ParentChart)->AxesList->Remove(axis);
		delete axis;
		series->ParentChart = 0;
		delete series;
	} else {
		series->Free();
	}
...
// always use free/delete to remove, never remove directly
void TMainGUI::free_ap_series(const int index) {
	...
	for( int i = 0; i < Chart_ap->SeriesCount(); i++ ) {
		if( !CompareStr(Chart_ap->Series[i]->Name, id) ) {
			delete_chart_series(Chart_ap->Series[i]);
			break;
		}
	}
}
...
{
	if(btn_multiple_axis->Tag) {
		// don't need to uncheck, cause function always called after uncheck
		free_ap_series(index);
        redraw_all_ap_series();
	} else {
		free_ap_series(index);
		zoom_reset_btnClick(0);
...
// just free series, do not uncheck or some
void TMainGUI::free_all_ap_series() {
	// every series should be removed separately
	while(Chart_ap->SeriesCount()) {
        delete_chart_series(Chart_ap->Series[0]);
    }
}

void TMainGUI::redraw_ap_series(const int index) {
	if(btn_multiple_axis->Tag) {
		redraw_all_ap_series();
	} else {
		free_ap_series(index);
...
void TMainGUI::redraw_all_ap_series() {
	// delete all to redraw from scratch
	// any series should not be removed directrly
	free_all_ap_series();
	// redraw from scratch
...
{
	std::vector<int> checked_series; // индексы элементов вектора параметров,
								// которые отмечены
...
void TMainGUI::enable_both_side_axis(bool enable) {
	btn_both_side_axis->Enabled = enable;
}
...
void TMainGUI::both_side_axis_set_off()
{
	btn_both_side_axis->Kind = bkOK;
	btn_both_side_axis->Caption = "One side";
	btn_both_side_axis->Tag = 0;
	btn_both_side_axis->Default = false;
}

void TMainGUI::both_side_axis_set_on()
{
	btn_both_side_axis->Kind = bkAll;
	btn_both_side_axis->Caption = "Both sides";
	btn_both_side_axis->Tag = 1;
}

void __fastcall TMainGUI::btn_both_side_axisClick(TObject *Sender)
{
	if(data_loaded()) {
		if(btn_both_side_axis->Tag) {
			both_side_axis_set_off();
		} else {
			both_side_axis_set_on();
		}
		redraw_all_ap_series();
		output_label->Caption = ""; // clear
	}
...
void __fastcall TMainGUI::Chart_apUndoZoom(TObject *Sender) // for multiple axis
{
	...
	if(btn_multiple_axis->Tag) {
		...
		scale_chart_ap_ma(); // scale y to 90%
	}
}

// chart AP: scale all multiple vertical chart axes to 90%
void TMainGUI::scale_chart_ap_ma() {
	for(int s_i = 0; s_i < Chart_ap->SeriesCount(); s_i++) {
		scale_chart_axis(Chart_ap, Chart_ap->Series[s_i]->GetVertAxis);
	}
}

// chart AP: scale single vertical axis to 90%
void TMainGUI::scale_chart_ap_sa() {
	scale_chart_axis(Chart_ap, Chart_ap->LeftAxis);
}

// scale chart axis to 90%
void TMainGUI::scale_chart_axis(TChart* chart, TChartAxis* axis) {
	double max = chart->MaxYValue(axis);
	double min = chart->MinYValue(axis);
	double offset = (max - min) / 10;
	if(offset == 0) {
		offset = 1;
	} else if(offset < 0) {
		show_error("Can't scale chart: offset < 0 !" + FloatToStr(offset));
		offset = -offset;
	}
	try {
		// AxisException if max<min (so is important to set both at same time)
...
void TMainGUI::zoom_chart_axis(TChartAxis* axis, int percent) {
